...
from tumor_network import tumor_densenet121,tumor_inceptionv3,tumor_MLP
...
parser.add_argument('--backbone',default='tumor_MLP')
...
log_folder=args.log+'/log_auto_MLP'+log_time
...
if args.backbone=='tumor_densenet121':
    net=tumor_densenet121(args.pretrained)
if args.backbone=='tumor_MLP':
    net=tumor_MLP()


##autoencoder
encoder=torch.load('./log_03302255auto/autoencoder_en_model.pth')


##graph tensorboard model
# rand_input=torch.rand(args.batch_size,3,args.input_size,args.input_size)
...
def getdata():
    ...
    if args.dataset == 'cifar100':
        ...
        valset = torchvision.datasets.CIFAR100(root='./', train=False, download=False, transform=transform_val)
        numclass = 100
    ...
    return train_loader,val_loader,numclass
...
def validation(epoch):
    global  log,best_loss,best_acc,early_stop,stop
    train_loader, val_loader= tumor_dataset()
    net.eval()
    val_loss=0
    correct=0
    ...
    with torch.no_grad():
        ...
        if (val_loss / (batch_idx + 1)) < best_loss:
            ...
            torch.save(net, './{}/{}_model.pth'.format(log_folder, args.backbone))
            torch.save(net.state_dict(), './{}/{}_weight.pth'.format(log_folder, args.backbone))
